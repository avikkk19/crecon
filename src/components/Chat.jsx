...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imlqc2hmYWl5bGlkbGpqdXZycmJvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDEzMjIxNDgsImV4cCI6MjA1Njg5ODE0OH0.AiIOVRQeBNuv94vGPQ26FAYUWlyH4BJ6IqbaVmYvIWA";
...
function Chat() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (processedMsg.content && processedMsg.content.includes('[ATTACHMENT]')) {
  ...
  async function createBucketIfNotExists() {
    try {
      const { data, error } = await supabase.storage.getBucket('message_attachments');
      
      if (error && error.statusCode === 404) {
        // Bucket doesn't exist, create it
        const { error: createError } = await supabase.storage.createBucket('message_attachments', {
          public: true,
  ...
  async function uploadFile(file) {
    if (file.size > MAX_FILE_SIZE) {
      alert(`File size exceeds the limit of 8MB. Your file is ${(file.size / (1024 * 1024)).toFixed(2)}MB.`);
      return null;
    }
  ...
  async function sendMessage(e) {
    ...
    try {
      ...
      setMessages(messages => 
        messages.filter(msg => !msg.is_optimistic)
      );
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-screen bg-gray-900 text-gray-300">Loading...</div>;
  }
        ...
        <div>
          {profiles.length === 0 ? (
            <div className="p-4 text-gray-500">
              No users found. Make sure there are other users in your profiles table.
            </div>
          ) : (
              ...
              >
                <div className="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center text-white mr-3">
                  {profile.avatar_url ? (
                    ...
                    (profile.username || profile.full_name || 'User').charAt(0).toUpperCase()
                  )}
                </div>
                <div>
                  ...
                  {profile.username && profile.full_name && (
                    <div className="text-sm text-gray-400">{profile.full_name}</div>
                  )}
                </div>
              </div>
            ))
            ...
            <div className="bg-gray-800 p-4 border-b border-gray-700 flex items-center">
              <div className="w-10 h-10 bg-purple-600 rounded-full flex items-center justify-center text-white mr-3">
                {selectedUser.avatar_url ? (
                  ...
                  (selectedUser.username || selectedUser.full_name || 'User').charAt(0).toUpperCase()
                )}
              </div>
              <div>
                ...
                {selectedUser.username && selectedUser.full_name && (
                  <div className="text-sm text-gray-400">{selectedUser.full_name}</div>
                )}
              </div>
            </div>
                ...
                messages.map(message => (
                  ...
                  >
                    ...
                    >
                      ...
                      {message.attachment_url && renderAttachment(message.attachment_url)}
                      <div
                ...
                <div className="mb-2 p-2 bg-gray-700 rounded flex items-center justify-between">
                  <div className="flex items-center">
                    {filePreview.type === 'image' ? (
                      <img src={filePreview.url} alt="Preview" className="h-12 w-12 object-cover rounded mr-2" />
                    ) : (
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-2 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                    )}
                    <span className="text-gray-300 truncate" style={{ maxWidth: '200px' }}>{filePreview.name}</span>
                  </div>
                  <button 
